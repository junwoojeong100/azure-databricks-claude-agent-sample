...
PAT가 기본 설정 경로입니다. 정적 PAT 저장을 피하려는 사용자는 뒤의
[OAuth U2M 선택 항목](#선택-oauth-u2m-helper)을 사용할 수 있습니다.
...
기본 경로는 보호된 `.env`에서 PAT를 읽는 helper입니다.
...
정적 PAT를 로컬에 저장하지 않으려면 Databricks CLI로 OAuth U2M 로그인을 한 뒤 단기
token을 가져오는 helper로 교체할 수 있습니다.
...
#!/usr/bin/env bash
set -euo pipefail
databricks auth token --profile claude-code --output json |
...
$WorkspaceHost = (
    Get-Content .env |
    Where-Object { $_ -match '^DATABRICKS_HOST=' } |
    Select-Object -First 1
) -replace '^DATABRICKS_HOST=', ''

databricks auth login `
    --host $WorkspaceHost.Trim().Trim('"').Trim("'") `
    --profile claude-code
...
OAuth U2M helper를 선택했다면 `apiKeyHelper`를 `get-oauth-token.sh` 또는
`get-oauth-token.ps1` command로 지정합니다.
...
OAuth U2M 선택, macOS/Linux:
/Users/<user>/.claude-databricks/get-oauth-token.sh

OAuth U2M 선택, Windows:
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "C:\Users\<user>\.claude-databricks\get-oauth-token.ps1"
```
