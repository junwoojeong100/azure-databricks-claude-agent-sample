...
| `DATABRICKS_TOKEN` | 자동 설치기가 사용할 credential | 기본 시작은 PAT. OAuth U2M/M2M은 별도 helper 사용 |
...
PAT는 가장 쉽게 시작할 수 있는 credential입니다. 정적 PAT 저장을 피하려면
[OAuth U2M helper](claude-code-databricks-reference.md#5-선택-개인-사용자-oauth-u2m-helper)를
선택할 수 있고, 운영 자동화는
[서비스 주체 OAuth M2M helper](claude-code-databricks-reference.md#6-운영용-oauth-m2m-helper)를
사용합니다.
...
이 `.env` 기반 자동 설정은 PAT 경로입니다. 보안 요구가 높아지면
[OAuth U2M](claude-code-databricks-reference.md#5-선택-개인-사용자-oauth-u2m-helper)이나
[OAuth M2M](claude-code-databricks-reference.md#6-운영용-oauth-m2m-helper) helper로
교체할 수 있습니다.
...
자동 설정이 가장 쉬운 경로입니다. 기존 Claude Code 설정을 timestamp 백업한 뒤 필요한
...
| HTTP 200이지만 `type: "message"`가 없음 | `/serving-endpoints/anthropic/v1/messages` 경로, Claude 모델 ID, 응답 body 확인 |

OAuth U2M/M2M, 모델 fallback, credential 충돌, custom base URL 제한은
