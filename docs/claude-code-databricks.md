...
`CLAUDE_CONFIG_DIR`가 없을 때 기본 실행은 사용자 전역 `~/.claude/settings.json`을
수정하므로 모든 프로젝트의 Claude Code가 Databricks를 사용합니다. 해당 환경변수가
있으면 그 디렉터리의 `settings.json`을 기본 대상으로 사용합니다. 이 동작을 원하는
경우 다음 기본 명령을 사용합니다.
...
제외합니다. 단, 리포 로컬 settings는 셸의 `ANTHROPIC_BASE_URL`, credential, model
override, `CLAUDE_CODE_USE_*` provider selector나 사용자 `~/.claude/settings.json`에서
상속되는 같은 `env` 키를 자동으로 지우지 않습니다.
...
설정 후에도 새 셸의 프로필이 Anthropic credential을 다시 export한다면 Claude Code를
다음처럼 credential을 제거한 프로세스로 시작합니다.
...
사용자 `settings.json`의 `env`에도 해당 override/provider selector가 있다면 제거하거나
[격리된 `CLAUDE_CONFIG_DIR` 사용](claude-code-databricks-reference.md#기존-anthropic-api-credential과-병행)을
따르세요. 그렇지 않으면 설치기의 격리된 종단 간 테스트는 성공해도 일반 실행에서는
기존 credential이 `apiKeyHelper`보다 먼저 사용될 수 있습니다.
...
5. 선택한 settings 파일(기본 `~/.claude/settings.json` 또는
   `$CLAUDE_CONFIG_DIR/settings.json`)에 URL과 모델 프리셋 병합
...
Foundry, Bedrock, Vertex, Mantle, Anthropic on AWS provider selector도 선택한 settings에서
제거해 Databricks base URL을 우회하지 않게 합니다.
...
unset ANTHROPIC_DEFAULT_OPUS_MODEL ANTHROPIC_DEFAULT_SONNET_MODEL
unset ANTHROPIC_DEFAULT_HAIKU_MODEL CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS
```
...
| 다른 provider/host/credential/model이 사용됨 | 셸/프로필의 ambient `CLAUDE_CODE_USE_*` selector와 `ANTHROPIC_*` override를 제거 후 재실행 |
