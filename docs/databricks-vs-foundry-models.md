...
| 추론 권한 | endpoint `CAN QUERY`; Foundation Model UC 권한 사용 시 대상 `system.ai` 모델 `EXECUTE` | Foundry resource 범위의 `Cognitive Services User` |
...
추론 권한은 같은 역할이 아닙니다. 모델 배포의 `Contributor`/`Owner`는 resource group
범위이고, Marketplace offer 구독 권한은 subscription 범위이므로 scope도 구분하세요.
...
| Agent 서비스 | 직접 구성 또는 Databricks agent 기능 | Microsoft Agent Framework로 직접 구성하거나 Foundry Agent Service BYOM gateway 사용 |

Foundry Claude 자체 호출은 Anthropic SDK/REST를 사용합니다. Foundry Agent Service의
기본 모델 지원은 Azure OpenAI와 Azure가 판매하는 일부 Foundry 모델이며, Anthropic이
판매하는 Claude는 표준 agent 생성 흐름의 기본 모델이 아닙니다. Claude agent는
Microsoft Agent Framework로 직접 구성하거나, 필요한 경우 Foundry Agent Service의
Bring Your Own Model(BYOM) gateway로 연결하세요. hosted tools와 evaluation도 별도
제품 계층이므로 deployment별 지원 범위를 확인해야 합니다.
...
| 모델 호출 | endpoint `CAN QUERY`; Foundation Model UC 권한 사용 시 모델 `EXECUTE` | `Cognitive Services User` |
