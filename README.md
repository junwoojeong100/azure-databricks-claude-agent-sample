...
1. Azure Databricks 워크스페이스에서 Claude Opus 4.8 Foundation Model API
   ...
   - Anthropic(Claude) 모델은 **파트너 기반(partner-powered) 모델**이라, 새로 만든
     계정/워크스페이스에서는 `databricks-claude-*` 호출이
     `403 PERMISSION_DENIED: ... Databricks-set rate limit of 0`으로 막힐 수 있습니다.
     이 경우 **계정 관리자**가 [account console](https://accounts.azuredatabricks.net) →
     **Settings → Feature enablement → Enable partner-powered AI features**를 **On**으로
     설정해야 합니다. (Databricks 자체 호스팅 오픈 모델은 이 설정 없이도 동작합니다.)
