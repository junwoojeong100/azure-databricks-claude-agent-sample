...
> 💻 **[Claude Code](https://code.claude.com/)(Anthropic의 터미널용 코딩 에이전트 CLI)에서**
> 이 Databricks Claude 엔드포인트를 백엔드로 쓰고
> 싶다면 [docs/claude-code-databricks.md](docs/claude-code-databricks.md)를 참고하세요.
> 원클릭 스크립트 [`scripts/setup_claude_code_databricks.sh`](scripts/setup_claude_code_databricks.sh)로
> 로컬 변환 프록시 설치까지 자동화됩니다.
...
테스트까지 한 번에 자동화하는 멱등(idempotent) 스크립트를 제공합니다.
...
`mlflow/v1/chat/completions`, `anthropic/v1/messages` 등을 노출하며,
...
> 참고: 실제 샘플의 `_rewrite_to_invocations` 훅은 경로 리라이트에 더해, Databricks가
> 거부하는 OpenAI `stream_options` 필드와 Anthropic이 거부하는 메시지 `name` 필드도
> 함께 제거합니다(→ [§5 문제 해결](#5-문제-해결-troubleshooting)의 "샘플이 자동 처리하는 항목").
...
- [Claude Code에서 Azure Databricks의 Claude 모델 사용하기 — 로컬 변환 프록시 설치·설정 가이드](docs/claude-code-databricks.md)
