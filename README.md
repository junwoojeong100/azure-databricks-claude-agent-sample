...
모델은 계정/지역별 서빙 용량 할당 대상**이라, 완전 오픈 모델(Llama 등)과 달리 할당이 0이면
이렇게 막힙니다. 같은 Entra 테넌트(= 같은 Databricks 계정)면 리전·워크스페이스를 바꿔도 동일합니다.
...
- **Azure Databricks account team에 문의**해 계정의 Anthropic 서빙 용량 활성화를 요청하세요
  (통제된 샌드박스/관리형 구독은 파트너 모델 용량이 기본 미할당일 수 있습니다).
